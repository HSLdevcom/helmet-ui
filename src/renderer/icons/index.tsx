import React from "react";

export const ArrowDown = () => {
  return (
    <svg version="1.1" id="Layer_1" x="0px" y="0px"
	 viewBox="0 0 283.46 283.46" enableBackground="new 0 0 283.46 283.46" xmlSpace="preserve">
      <g>
        <path fill="#007AC9" d="M-0.003,248.028c0,19.488,15.945,35.434,35.434,35.434h212.598
          c19.489,0,35.434-15.946,35.434-35.434V35.43c0-19.488-15.945-35.432-35.434-35.432H35.431c-19.489,0-35.434,15.944-35.434,35.432
          V248.028z"/>
      </g>
      <g>
        <path fill="#FFFFFF" d="M56.118,149.487c0,0,77.177,77.182,77.839,77.843c0.731,0.661,3.421,3.221,7.768,3.221
          c4.348,0,7.106-2.56,7.758-3.212l0.002-0.002l77.848-77.85c4.285-4.285,4.285-11.232,0-15.52
          c-4.285-4.283-11.234-4.283-15.52,0.002l-59.117,59.118l0.003-129.201c0-6.06-4.913-10.972-10.974-10.972
          c-6.06,0-10.974,4.912-10.974,10.972v129.201l-59.112-59.118c-4.286-4.285-11.233-4.285-15.519-0.002
          C51.835,138.255,51.835,145.202,56.118,149.487"/>
      </g>
    </svg>
    )
}

export const ArrowRight = () => {
  return (
    <svg version="1.1" id="Layer_1" x="0px" y="0px"
	 viewBox="0 0 283.46 283.46" enableBackground="new 0 0 283.46 283.46;">
      <g>
        <path fill="#007AC9" d="M-0.003,248.028c0,19.488,15.945,35.434,35.434,35.434h212.598
          c19.489,0,35.434-15.946,35.434-35.434V35.43c0-19.488-15.945-35.432-35.434-35.432H35.431c-19.489,0-35.434,15.944-35.434,35.432
          V248.028z"/>
      </g>
      <g>
        <path fill="#FFFFFF" d="M149.484,227.338c0,0,77.182-77.177,77.843-77.839c0.661-0.731,3.221-3.421,3.221-7.768
          c0-4.348-2.56-7.106-3.212-7.758l-0.002-0.002l-77.85-77.848c-4.285-4.285-11.232-4.285-15.52,0
          c-4.283,4.285-4.283,11.234,0.002,15.52l59.118,59.117l-129.201-0.003c-6.06,0-10.972,4.913-10.972,10.974
          c0,6.06,4.912,10.974,10.972,10.974h129.201l-59.118,59.112c-4.285,4.286-4.285,11.233-0.002,15.519
          C138.252,231.621,145.199,231.621,149.484,227.338"/>
      </g>
    </svg>
    );
};

export const ArrowUp = () => {
  return (
    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
      viewBox="0 0 283.46 283.46" enableBackground="new 0 0 283.46 283.46" xmlSpace="preserve">
      <g>
        <path fill="#007AC9" d="M-0.003,248.028c0,19.488,15.945,35.434,35.434,35.434h212.598
          c19.489,0,35.434-15.946,35.434-35.434V35.43c0-19.488-15.945-35.432-35.434-35.432H35.431c-19.489,0-35.434,15.944-35.434,35.432
          V248.028z"/>
      </g>
      <g>
        <path fill="#FFFFFF" d="M227.336,133.975c0,0-77.177-77.182-77.839-77.843c-0.731-0.661-3.421-3.221-7.768-3.221
          c-4.348,0-7.106,2.56-7.758,3.212l-0.002,0.002l-77.848,77.85c-4.285,4.285-4.285,11.232,0,15.52
          c4.285,4.283,11.234,4.283,15.52-0.002l59.117-59.118l-0.003,129.201c0,6.06,4.913,10.972,10.974,10.972
          c6.06,0,10.974-4.912,10.974-10.972V90.375l59.112,59.118c4.286,4.285,11.233,4.285,15.519,0.002
          C231.619,145.207,231.619,138.26,227.336,133.975"/>
      </g>
    </svg>
    )
}

export const CopyIcon = () => {
  return (
    <svg width="19" height="22" viewBox="0 0 19 22" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path fill="#007AC9" d="M14 0H2C0.9 0 0 0.9 0 2V16H2V2H14V0ZM17 4H6C4.9 4 4 4.9 4 6V20C4 21.1 4.9 22 6 
          22H17C18.1 22 19 21.1 19 20V6C19 4.9 18.1 4 17 4ZM17 20H6V6H17V20Z"/>
    </svg>
    )
}

interface IconProps extends React.SVGProps<SVGSVGElement> {}

export const ResetIcon: React.FC<IconProps> = (props) => {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 283.46 283.46" width="20" height="20">
      <path fill="#007ac9" d="M-.002 248.028c0 19.488 15.945 35.435 35.434 35.435h212.597c19.489 0 35.435-15.946 
        35.435-35.435V35.431c0-19.488-15.945-35.433-35.435-35.433H35.432C15.943-.002-.002 15.943-.002 35.431v212.597z"/>
      <path fill="#fff" d="M55.29 218.302l63.09-78.255-59.45-73.706c-3.033-3.943-5.763-8.796-5.763-13.953 0-10.313 
        7.886-18.199 18.502-18.199 7.886 0 12.436 3.033 16.985 9.099l53.687 70.672 53.08-69.459c4.853-6.673 10.009-10.313 
        18.198-10.313s16.985 6.673 16.985 16.985c0 5.46-2.123 9.706-5.763 14.256l-59.449 74.009 62.482 77.952c3.033 3.943 
        5.763 8.493 5.763 13.952 0 10.314-7.886 18.199-18.502 18.199-7.886 0-12.435-3.033-16.985-9.1l-57.023-74.312-56.112 
        73.101c-5.156 6.673-10.009 10.313-18.199 10.313-8.493 0-16.986-6.673-16.986-17.289.001-5.156 2.124-9.402 5.46-13.952z"/>
    </svg>
  )
}